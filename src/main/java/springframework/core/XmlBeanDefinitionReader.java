...
public class XmlBeanDefinitionReader {
    protected static Logger log = Logger.getLogger(BeanProcesser.class
            .getName());
    ...
    public static Map<String, Bean> readXmlFile(String fileName) {
        ...
        while (beanIterator.hasNext()) {
            ...
            Iterator<?> beanProperties = beanElm.elementIterator("property");
            while (beanProperties.hasNext()) {
                ...
                //      <value>"kitty"</value>
                //  </property>
                if (beanProperty.element("value") != null) {
                    String value = beanProperty.elementText("value");
                    bean.getProperties().put(name, value);
                }
                // 如果是普通赋值 value形式
                // <property name="myDog" value="wangwang" />
                if (beanProperty.attributeValue("value") != null) {
                    String value = beanProperty.attributeValue("value");
                    bean.getProperties().put(name, value);
                }

                // 如果是引用另外一个bean赋值 ref形式
                if (beanProperty.element("ref") != null) {
                    String[] refString = new String[] { beanProperty.element(
                            "ref").attribute("bean").getValue() };
